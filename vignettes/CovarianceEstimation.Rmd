...
model <- isdccfit(etfdata, numRegimes=3, paralletType = 0, itermax = 5)
...
pspec.lo <- portfolio.spec(assets = colnames(etfdata))
...
```{r isdcc-run, warning=FALSE, error=FALSE , message=FALSE}

opt.ordinary <- 
  optimize.portfolio.rebalancing(etfdata, pspec.lo, 
                                 optimize_method="quadprog",
                                 rebalance_on="months", 
                                 training_period=120,
                                 trailing_periods=120)
...
ordinary.wts <- na.omit(extractWeights(opt.ordinary))
ordinary <- Return.rebalancing(R=etfdata, weights=ordinary.wts)

dcc.wts <- na.omit(extractWeights(opt.dcc))
dcc <- Return.rebalancing(R=etfdata, weights=dcc.wts)

isdcc.wts <- na.omit(extractWeights(opt.isdcc))
isdcc <- Return.rebalancing(R=etfdata, weights=isdcc.wts)

strat.rets <- merge.zoo(merge.zoo(ordinary, dcc), isdcc)
colnames(strat.rets) <- c("ordinary", "dcc", "isdcc")
